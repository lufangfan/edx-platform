...
def _set_jwt_cookie(response, request):
...
def _set_jwt_expiration(cookie_settings):
    ...
    cookie_settings['expires'] = _cookie_expires_from_max_age(max_age)


def _build_jwt(request, expires_in):
    """ Builds and returns a JWT. """
    jwt_builder = JwtBuilder(request.user, asymmetric=True)
    scopes = ['email', 'profile']
    additional_claims = {
        'filters': [],
        'is_restricted': False,
    }
    return jwt_builder.build_token(scopes, expires_in, additional_claims=additional_claims)
